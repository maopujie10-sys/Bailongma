// Agent 委派 / 工具市场类 schema：delegate_to_agent / dispatch_to_army / grant_agent_delegation /
...
export const agentsSchemas = {
  ...
  dispatch_to_army: {
    type: 'function',
    function: {
      name: 'dispatch_to_army',
      description: '派任务给 Python Agent 军团执行。支持三个引擎：crewai（多Agent协作编排）、metagpt（软件公司多Agent协作）、browser-use（浏览器操控）。适合需要多Agent协作、自动化浏览器操作、或复杂软件工程的场景。',
      parameters: {
        type: 'object',
        properties: {
          engine: {
            type: 'string',
            description: '执行引擎：crewai（多Agent协作）、metagpt（软件工程）、browser-use（浏览器操控）。',
            enum: ['crewai', 'metagpt', 'browser-use']
          },
          task: {
            type: 'string',
            description: '任务描述，越具体越好。例如"写一个Python爬虫抓取豆瓣电影Top250"或"打开百度搜索今天天气"。'
          },
          context: {
            type: 'string',
            description: '可选：附加上下文信息，会拼接到 task 前面。'
          },
          timeout: {
            type: 'number',
  ...
  pull_agent_from_github: {
    type: 'function',
    function: {
      name: 'pull_agent_from_github',
      description: '从 GitHub 仓库拉取 Agent 或 Skill。支持公开仓库，自动扫描 SKILL.md 和 AGENTS.md 并注册。',
      parameters: {
        type: 'object',
        properties: {
          repo_url: {
