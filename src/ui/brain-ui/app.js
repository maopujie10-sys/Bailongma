...
function initTTSSettings() {
  ...
  // Edge TTS 语速/音调
  const edgettsRateEl = document.getElementById("tts-edgetts-rate");
  const edgettsRateVal = document.getElementById("tts-edgetts-rate-val");
  const edgettsPitchEl = document.getElementById("tts-edgetts-pitch");
  const edgettsPitchVal = document.getElementById("tts-edgetts-pitch-val");
  if (edgettsRateEl) {
    edgettsRateEl.addEventListener("input", () => {
      if (edgettsRateVal) edgettsRateVal.textContent = (parseInt(edgettsRateEl.value, 10) || 0) + "%";
    });
  }
  if (edgettsPitchEl) {
    edgettsPitchEl.addEventListener("input", () => {
      if (edgettsPitchVal) edgettsPitchVal.textContent = (parseInt(edgettsPitchEl.value, 10) || 0) + "%";
    });
  }
  ...
  const credSections = {
    edgetts:    document.getElementById("tts-creds-edgetts"),
    aliyun:     document.getElementById("tts-creds-aliyun"),
    tencent:    document.getElementById("tts-creds-tencent"),
  ...
  fetch(`${API}/settings/tts`).then(r => r.json()).then(({ tts, voices }) => {
    ...
    // Edge TTS
    const erEl = document.getElementById("tts-edgetts-rate");
    if (erEl && tts?.edgettsRate !== undefined) {
      erEl.value = tts.edgettsRate;
      const erv = document.getElementById("tts-edgetts-rate-val");
      if (erv) erv.textContent = (Number(tts.edgettsRate) || 0) + "%";
    }
    const epEl = document.getElementById("tts-edgetts-pitch");
    if (epEl && tts?.edgettsPitch !== undefined) {
      epEl.value = tts.edgettsPitch;
      const epv = document.getElementById("tts-edgetts-pitch-val");
      if (epv) epv.textContent = (Number(tts.edgettsPitch) || 0) + "%";
    }
  ...
  if (origSaveBtn) {
    origSaveBtn.addEventListener("click", () => {
      ...
      const er = document.getElementById("tts-edgetts-rate")?.value;
      if (er !== undefined && er !== "0") ttsBody.edgettsRate = Number(er);
      const ep = document.getElementById("tts-edgetts-pitch")?.value;
      if (ep !== undefined && ep !== "0") ttsBody.edgettsPitch = Number(ep);
      const ak = document.getElementById("tts-aliyun-appkey")?.value?.trim();
      if (ak) ttsBody.aliyunAppKey = ak;
      const ta = document.getElementById("tts-tencent-appid")?.value?.trim();
      if (ta) ttsBody.tencentAppId = ta;
      const ts = document.getElementById("tts-tencent-secret-id")?.value?.trim();
      if (ts) ttsBody.tencentSecretId = ts;
      const tk = document.getElementById("tts-tencent-secret-key")?.value?.trim();
      if (tk) ttsBody.tencentSecretKey = tk;
      ...
      fetch(`${API}/settings/tts`, {
        ...
        ["tts-minimax-key", "tts-doubao-key", "tts-doubao-access-key", "tts-openai-key", "tts-elevenlabs-key", "tts-volcano-token", "tts-aliyun-appkey", "tts-tencent-secret-key"].forEach(id => {
