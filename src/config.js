...
export const MIMO_MODELS = [
  ...
  {
    // 极速版：保留为可选项，非默认首选（小米平台暂无此官方ID，调用失败会自动降级到上面的真实模型）    id: 'MiMo-V2.5-Pro-UltraSpeed',
...
// 作用：升级后 config.json 里的旧 provider 名不会再让整个 LLM 配置作废（见下方分块容错加载），
...
// 只负责把 config.json 解析成对象；文件缺失或损坏才返回 null.?// 不在这里判断 LLM 块是否可用——那是加载逻辑的事，避.?一个字段不合法就丢掉整份文件.?// 连带.?voice/tts/security 等兄弟字段一起重.?（升级后最常见.?配置全没.?根因）.
...
// 判断旧版 config.json 里的 LLM 块能否直接激活（provider/apiKey/custom 三件套齐全）.?// 返回规整后的 { provider, apiKey, model, baseURL }（provider 已过别名映射）；不可用则返回 null.?
...
// 读出 config.json 现有内容（失败返回空对象）.?// activate() 等写操作必须基于它合并，否则会抹.?voice/tts/security 等其它字段.?
function readExistingStoredConfig() {
  try { return JSON.parse(fs.readFileSync(paths.configFile, 'utf-8')) || {} }
  catch { return {} }
}

// .?写时必合并、绝不全量覆.?变成不可绕过的约束，杜绝再次出现"改一个字段抹掉其它块".?// 注意：浅合并无法删除键；需要删字段.?setter 仍自.?readExistingStoredConfig + 解构剔除.?writeStoredConfig.?
...
// ── config.json schema 版本与迁.?──
// .?db.js 的迁移规范：config.json 带一.?schemaVersion 字段，启动时按版本号顺序跑迁移.?// 跑完写回新版本号。把历史上零散、惰性触发的"一次性迁.?（如 seedance 拆分）收编到这里.?// 让升级路径确定、可测、可追溯，而不是散落在.?getter 里.?// 加新迁移：CONFIG_SCHEMA_VERSION .?1，并.?CONFIG_MIGRATIONS 里补上对应版本号的函数.?
const CONFIG_SCHEMA_VERSION = 2

// 每个迁移把传入的 config 对象升一级，返回新对象。允许带幂等副作用（如写独立文件）.?
const CONFIG_MIGRATIONS = {
  // v0 .?v1：把旧版塞在 config.json 里的 seedance 块拆到独立的 seedance.json.?  // 并从主配置移除该字段。等价于 migrateLegacySeedance，收编为正式、确定性的启动迁移.?
  1(cfg) {
  ...
  // v1 .?v2：LLM 凭据.?provider 拆到 userData/llm/<provider>.json.?  // config.json 只保留当.?provider 指针.?temperature/security/voice 等通用块.?
  2(cfg) {
...
// 启动时执行一次。文件缺.?损坏则跳过（无可迁移）；任一迁移抛错则中止且不写回，
...
// 迁移必须在下面读.?加载 config.json 之前跑完，确保后续逻辑看到的是已升级的结构.?runConfigMigrations()

// 加载顺序刻意分块容错：先无条件吃.?temperature / security .?兄弟字段".?// 再单独判.?LLM 块能否激活。这样即.?LLM 块因 provider 改名/缺字段而不可用.?// 也不会连带把沙盒开关、温度等其它配置一起重置——升级后最常见.?配置全没.?根因.?
...
// ── Seedance AI 视频生成（火山方.?Ark）配.?──
// 存于 config.json .?seedance 字段：{ apiKey, model, baseURL }
// 中国区默认走 ark.cn-beijing.volces.com；model .?doubao-* 形态的模型 ID 或推理接入点 ep-xxx.?// 因不同账号开通的版本号不同，做成可配置，给一个合理默认值，错了由调用错误回传引导用户改.?
...
// 一次性迁移：旧版.?seedance 存在 config.json 里。若独立文件尚无、.?config.json 里还有，
...
const TTS_CONFIG_KEYS = [
  ...
  'edgettsRate', 'edgettsPitch',
  'aliyunAppKey', 'aliyunApiKey',
  'tencentAppId', 'tencentSecretId', 'tencentSecretKey',
  'minimaxKey',
...
export function getTTSConfig() {
  ...
  return {
    ttsProvider:     stored.ttsProvider  || 'edgetts',
    ttsVoiceId:      stored.ttsVoiceId   || 'zh-CN-XiaoxiaoNeural',
    ...
    edgettsRate:     Number(stored.edgettsRate || 0) || 0,
    edgettsPitch:    Number(stored.edgettsPitch || 0) || 0,
    ttsEnabled:      stored.ttsEnabled !== undefined ? stored.ttsEnabled : '1',
    aliyunAppKey:    { configured: !!(stored.aliyunAppKey) },
    tencentSecretId: { configured: !!(stored.tencentSecretId) },
  }
}
...
export function getTTSCredentials() {
  ...
  stored = existing?.tts || {}
  return {
    provider:       stored.ttsProvider  || 'edgetts',
    voiceId:        stored.ttsVoiceId   || 'zh-CN-XiaoxiaoNeural',
    edgettsRate:    Number(stored.edgettsRate ?? 0) || 0,
    edgettsPitch:   Number(stored.edgettsPitch ?? 0) || 0,
    aliyunAppKey:       stored.aliyunAppKey     || '',
    aliyunApiKey:       stored.aliyunAppKey     || existing.voice?.aliyunApiKey || '',
    aliyunAccessKeyId:  stored.aliyunAccessKeyId || '',
    aliyunAccessKeySecret: stored.aliyunAccessKeySecret || '',
    aliyunToken:        stored.aliyunToken       || '',
    tencentAppId:       stored.tencentAppId      || '',
    tencentSecretId:    stored.tencentSecretId   || '',
    tencentSecretKey:   stored.tencentSecretKey  || '',
...
// ── Embedding config ──────────────────────────────────────────────────────────
// Embedding .?chat provider 完全独立。DeepSeek/Moonshot .?embedding API.?// 所以必须分开存。结构：config.json .?"embedding" 块.?//
// 字段.?//   provider:   'openai' | 'qwen' | 'zhipu' | 'minimax' | 'custom'
//   model:      模型名（参.?EMBEDDING_PROVIDER_PRESETS.?//   apiKey:     凭证（明文存储，与现.?chat apiKey 一样）
//   baseURL:    custom 时必填；其他 provider 留空走预.?//   dimensions: 可选，.?OpenAI text-embedding-3-* 系列支持显式指定
...
function readEmbeddingBlock() {
  ...
  try {
    ...
    // config 文件不存在或访问失败：直接返.?{}，不缓存（让下次有机会重试）
    return {}
  }
...
// Backend-only：读明文 apiKey。供 src/embedding.js 内部用，不要给前端.?export
...
// 前端可见视图：不暴露 key 明文，只暴露 configured 布尔 + searxngUrl（URL 不算敏感.?// configured 同时考虑 env 兜底，避.?env 里有 key .?UI 标未配置"的误.?// xxxFromEnv 提示来源，让 UI 标注"已配置（环境变量.?，并暗示清空输入框不会真正生.?
export function getWebSearchConfig() {
  ...
  return {
    ...
    // 输入框只回显 stored 值，避免用户以为能编.?env .?    searxngUrl:       stored.searxngUrl,
...
// Backend-only：读明文 key。供 src/capabilities/executor.js 内部用，不要给前.?
