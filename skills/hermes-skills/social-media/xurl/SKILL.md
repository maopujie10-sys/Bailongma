...
version: 1.1.1
author: xdevplatform + openclaw + Bailongma (白龙马)
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Bailongma (白龙马) tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Bailongma (白龙马) Docker layout, `/opt/data` is `BAILONGMA_USER_DIR`, but Bailongma (白龙马) tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Bailongma (白龙马)-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Bailongma (白龙马) tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use Bailongma (白龙马) subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Bailongma (白龙马) adaptation: reformatted for Bailongma (白龙马) skill conventions; safety guardrails preserved verbatim.
