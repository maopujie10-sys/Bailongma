...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _bailongma_env="${BAILONGMA_USER_DIR}/.env"; [ -f "$_bailongma_env" ] && grep -q "^GITHUB_TOKEN=" "$_bailongma_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_bailongma_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Bailongma (白龙马)\",
    \"comments\": [
...
```bash
source "${BAILONGMA_USER_DIR}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Bailongma (白龙马). Code looks clean — good test coverage, no security concerns."
